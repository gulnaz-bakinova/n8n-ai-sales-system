...
В проекте реализованы паттерны уровня Senior Automation Engineer для обеспечения выживаемости системы в Production:
...
2. Настройте credentials (или используйте mock-данные), опираясь на файл `.env.example`.
3. Откройте воркфлоу **`90_test_mock_provider`**.
4. Скопируйте любой JSON-payload из папки `/fixtures` в ноду HTTP Request и выполните запуск.
5. Откройте **`01_core_ai_sales_agent`**, чтобы наблюдать за маршрутизацией State Machine, работой RAG и парсингом JSON-ответов от ИИ.
