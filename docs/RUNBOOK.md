...
All unhandled exceptions are caught by the `99_global_incident_handler` workflow, which logs the error to the Google Sheets DLQ (Dead Letter Queue) and sends a Telegram alert.
...
**Automatic Mitigation:**
- Critical read nodes have built-in n8n retries.
**Manual Resolution:**
